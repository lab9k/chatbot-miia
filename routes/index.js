...
function sendResponse(agent, question, body, goodanswer = false) {
    ...
    if (
        (!goodanswer &&
            pars.length > 0 &&
            pars[0].score &&
            pars[0].score > UPPER_BOUND_SCORE) ||
        (!goodanswer &&
            highestScoring.score &&
            highestScoring.score > UPPER_BOUND_SCORE)
    ) {
    ...
    agent.add(
        responses.query_followup.nl[
            Math.floor(Math.random() * responses.query_followup.nl.length)
        ]
    );
}
...
function sendErrorResponse(agent) {
    agent.add(
        responses.error.nl[
            Math.floor(Math.random() * responses.error.nl.length)
        ]
    );
}
...
function getDescription(item) {
    ...
    if (item.summary && item.summary !== null) {
        ...
        description =
            item.content.length <= MAX_DESCRIPTION_LENGTH
                ? item.content
